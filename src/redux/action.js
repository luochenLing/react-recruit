...
function initIO(dispatch, userid) {
  if (!io.socket) {
    ...
    io.socket.on("receiveMsg", function(chatMsgs) {
      console.log("客户端接收服务端发送的消息", chatMsgs);
      if (userid === chatMsgs.from || userid === chatMsgs.to) {
        dispatch(receiveMsg(chatMsgs, userid));
      }
    });
  }
}
...
export const register = user => {
  ...
  return async dispatch => {
    const result = await (await reqRegister({ username, password, type })).data;
    if (result.code === 0) {
...
export const login = user => {
  ...
  return async dispatch => {
    const result = await (await reqLogin(user)).data;
    if (result.code === 0) {
      await getMsgList(dispatch, result.data._id);
      dispatch(authSuccess(result.data));
    } else {
